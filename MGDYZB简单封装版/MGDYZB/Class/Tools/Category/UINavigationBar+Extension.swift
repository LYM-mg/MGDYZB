...
extension UINavigationBar {
     ...
     */
    func change(_ color: UIColor, with scrollView: UIScrollView, andValue value: CGFloat,colorIsStartExit: Bool=false) {
        if scrollView.contentOffset.y < -value{
            ...
            self.isHidden = false
            //计算透明度
            var alpha: CGFloat = scrollView.contentOffset.y / value > 1.0 ? 1 : scrollView.contentOffset.y / value
            alpha = colorIsStartExit ? (1 - alpha) : alpha
            //设置一个颜色并转化为图片
